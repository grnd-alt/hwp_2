...
int main(){
	...
	//writing	
	drv.analogWrite1(812); // AA1 set to 0V - changing in loop
	//reading
	...
	for (int i = 0; i < 1023;i+=8){
		...
		double Uds = ((i*5.0)/1024) - U1;
		std::cout<< std::to_string(U1) + ";"+std::to_string(Uds)+";"+std::to_string(current)<<std::endl;
		filestring += std::to_string(U1) + ";" + std::to_string(Uds) + std::to_string(current) + "\n";
	}
	file << filestring;
	drv.analogWrite0(0);
	drv.analogWrite1(0);
