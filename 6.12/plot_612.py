...
def plotAllXYfrom3(filenames):
    ...
    for name in filenames:
        t = loadColFromFile(name,3)
        graphs.append(loadColFromFile(name,2))
        plt.plot(t,graphs[-1],label=name)
    plt.xlabel("Basis Strom")
    plt.ylabel("Collector Emitter Strom")
    plt.legend()
    plt.show()
if __name__ == "__main__":
    plotAllXYfrom3(['./hwp_2/6.12/dataMos2.txt','./hwp_2/6.12/dataMos3.txt','./hwp_2/6.12/dataMos4.txt','./hwp_2/6.12/dataMos5.txt'])
